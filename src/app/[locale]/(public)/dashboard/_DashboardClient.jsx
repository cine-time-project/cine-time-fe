...
const TILES = [
  // Anahtarlar (key) değişmedi; ikonlar bu key’lere bağlı
...
const normalizeRoles = (arr = []) =>
  arr.map((r) => String(r).toUpperCase().replace(/^ROLE_/, ""));
const hasAnyRole = (userRoles = [], allowed = []) => {
  ...
  switch (name) {
    ...
    case "cities":
      return (
        <svg {...common}>
          <path d="M3 21h18" />
          <path d="M4 21v-7l4-2 4 2v7" />
          <path d="M12 21v-10l4-2 4 2v10" />
        </svg>
      );
    case "images":
      return (
...
export default function DashboardClient({ locale }) {
  ...
  const tiles = useMemo(
    () =>
      TILES.map((tile) => {
        // i18n: varsa çeviriyi kullan, yoksa sabit metne düş
        ...
        try {
          ...
          if (maybeTitle && maybeTitle !== titleKey) title = maybeTitle;
        } catch {}
        try {
          const maybeDesc = t(descKey);
          if (maybeDesc && maybeDesc !== descKey) desc = maybeDesc;
        } catch {}

        return {
          ...tile,
          href: `/${locale}${tile.href}`,
          allowed: hasAnyRole(roles, tile.roles),
          title,
          desc,
        };
      }),
  ...
  try {
    const maybe = t("title");
    if (maybe && maybe !== "title") pageTitle = maybe;
  } catch {}
